...
let _forms: Record<string, Component> = {};
...
async function registerType(definition: NodeDefinition): Promise<void> {
  ...
  try {
    ...
    const defaults = nodeDefinition.configSchema
      ? getDefaultsFromSchema(nodeDefinition.configSchema)
      : undefined;
    const credentials = nodeDefinition.credentialsSchema
      ? getCredentialsFromSchema(nodeDefinition.credentialsSchema)
      : undefined;

    // Add validation toggle defaults when schemas exist
    if (defaults) {
      if (nodeDefinition.inputSchema) {
        defaults.validateInput = { required: false, value: false };
      }
      if (nodeDefinition.outputsSchema) {
    ...
    function oneditprepare(this: Node) {
      ...
      const form = definition.form ?? (_forms[type] ? { component: _forms[type] } : undefined);
