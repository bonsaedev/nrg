...
abstract class Node<TConfig = any, TCredentials = any, TSettings = any> {
  ...
  constructor(
    ...
    if (constructor.configSchema) {
      ...
      validator.validate(config, constructor.configSchema, {
        cacheKey:
          constructor.configSchema.$id || `${constructor.type}:configs-schema`,
        throwOnError: true,
      });
      this.log("Configs are valid");
    }
    ...
    if (constructor.credentialsSchema && credentials) {
      ...
      validator.validate(credentials, constructor.credentialsSchema, {
        cacheKey:
          constructor.credentialsSchema.$id ||
          `${constructor.type}:credentials-schema`,
        throwOnError: true,
      });
      this.log("Credentials are valid");
    }
  }
