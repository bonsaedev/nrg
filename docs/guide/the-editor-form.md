...
How each config, credential, or setting field renders in the editor form — the field-type catalog plus the `x-nrg-form` extensions, `TypedInput`, `NodeRef`, conditional validation, and type inference. To replace the generated form with a bespoke Vue component, see [Custom Vue Forms](./custom-vue-forms).
...
[Custom Vue Forms](./custom-vue-forms#the-editor-form) for exactly what surfaces each section.
