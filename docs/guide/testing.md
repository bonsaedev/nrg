...
Component tests run in a **real browser**. Never value-import the server runtime — and that includes your `src/schemas/*` modules, which import `defineSchema`/`SchemaType` from `@bonsae/nrg/server` (Node-only). Value-importing them pulls the node runtime into the browser bundle and crashes the test. Instead, pass your node's `type` to `createNode()` and let the [schemas globalSetup](#resolving-schemas-by-node-type) hand the real schema to the test as serialized data.
:::
