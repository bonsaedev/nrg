...
How nrg renders the editor form from your schema, and how to replace or extend it with a custom Vue component using `useFormNode` and the built-in form components. For the schema-driven field catalog, see [The Editor Form](./the-editor-form).
...
full Ports Settings section: the lifecycle toggles and the data-validation
controls. The data-validation controls are always there too: a
